...
class NewTimeout:
    def __init__(self, name, **kwargs):
        ...
        self.severity = kwargs.get('severity', 1)  # Changes how sharply the time increases
    ...
    async def is_on_timeout(self, **kwargs):
        ...
        if Id is not None:  # we check a specific user / thing's timeout
            if Id in self.time_outs:
                selected_timeout = self.time_outs[Id]
                if selected_timeout['time_stamp'] > datetime.now():
                    ...
                    max_time = timedelta(days=self.max_time[0],
...
async def raise_the_alarm(id_):
    print("We have someone spamming captain! user: {}".format(id_))


async def main():
    my_timeout_manager = TimeoutManager()
    my_new_timeout = NewTimeout(name="Test",
                                alert_on_spam=True,
                                alert_func=raise_the_alarm,
                                max_offences=3)

    my_timeout_manager.add_timeout(my_new_timeout)

    for _ in range(6):  # let just flood requests
        await my_timeout_manager.on_timeout('Test', Id=12345)
